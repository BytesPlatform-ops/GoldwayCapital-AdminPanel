...
/**
 * Server-side proxy: the public LeadForm posts here; we attach the shared ingest
 * key and forward to the backend form endpoint. Keeps the key out of the browser.
 */
...
export async function POST(req: NextRequest) {
  ...
  const res = await fetch(`${BACKEND}/api/forms/${source}`, {
    method: "POST",
